...
import styles from './TablePatientExamEnd.module.scss';
...
function TablePatientExamEnd() {
	const router = useRouter();

	const {accountId} = useSelector((state: RootState) => state.user);
	const {token} = useSelector((state: RootState) => state.auth);
	const {medicalStatus, medicalExam, medicalTime, medicalKeyword} =
	...
	useEffect(() => {
		httpRequest({
			...
			http: medicalRecord.getMedicalRecordFinish({
				token: token!,
				AccountUuid: accountId as string,
				Page: page,
				Limit: limit,
				Keyword: keyword || '',
				State: medicalStatus?.id,
				...
				<DataWrapper
					...
					noti={
						<Noti
							...
							des='Có vẻ như bạn chưa có phiếu khám nào đã kết thúc.'
						/>
					}
				>
...
export default TablePatientExamEnd;
